...
def nivel1_exercicio1():
    ...
    def crawl_endpoint(endpoint_url, resultados, index):
        ...
        try:
            ...
            print(f"[Thread {index}] {endpoint_url} → status {response.status_code}, tempo {latency:.2f}s")
    ...
    for idx, info in resultados.items():
        if info.get('status_code') is not None:
            print(f"[{idx}] {info['url']} → status {info['status_code']}, latência {info['latency']:.2f}s")
        else:
...
def nivel1_exercicio3():
    ...
    def testar_url(url, resultados, index):
        ...
        try:
            ...
            print(f"[Thread {index}] {url} → status {resp.status_code}, latência {latencia:.2f}s")
        except Exception as e:
            latencia = time.time() - start
            resultados[index] = {
                'url': url,
...
def nivel1_exercicio4():
    ...
    def simular_download(nome_arquivo):
        ...
        print(f"Iniciando download simulado de '{nome_arquivo}' por {tempo_simulado:.2f}s...")
...
def nivel1_exercicio5():
    ...
    def consultar_base(nome_base, resultados, index):
        ...
        print(f"[{nome_base}] Iniciando consulta (duração simulada: {tempo_simulado:.2f}s)...")
        time.sleep(tempo_simulado)
        ...
        print(f"[{nome_base}] Consulta concluída: {contagem_linhas} linhas em {tempo_simulado:.2f}s.")
    ...
    for idx in range(len(bases)):
        info = resultados[idx]
        print(f"{info['base']}: {info['linhas_lidas']} linhas lidas (tempo {info['duracao']:.2f}s)")
...
def nivel2_exercicio6():
    ...
    # Import de math aqui porque usar dentro da função
    import math
    ...
    blocos = [dados[i * tamanho_bloco:(i + 1) * tamanho_bloco] for i in range(num_processos)]
    ...
    print(f"Transformação concluída. Total de elementos processados: {len(resultados)} (Exercício 6)")
...
def nivel2_exercicio7():
    ...
    def funcao_complexa(df_slice):
        df = df_slice.copy()
        df['Resultado'] = np.sqrt(df['X']**2 + df['Y']**2) + np.log(df['X'] + df['Y'] + 1)
        return df
...
def nivel2_exercicio9():
    ...
    agg_final = df_concat.groupby('Chave').agg({
        'sum': 'sum',
        'mean': 'mean',
        'std': 'mean'
    }).reset_index()
...
def nivel2_exercicio10():
    ...
    def processo_persistencia(fila_out, pasta_saida):
        ...
        while True:
            ...
            print(f"[Persistência] Gravado: {nome_arquivo} ({len(df_transformado)} linhas).")
            contador += 1
    ...
    for i in range(num_blocos):
        ...
        print(f"[Principal] Enviando bloco {i} com {len(df)} linhas para transformação.")
        fila_in.put(df)
